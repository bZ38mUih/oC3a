...
$appRJ->response['result'].= "<a href='/gallery/glManager/newCat'><img src='/source/img/create-icon.png'>Создать категорию</a>";
$appRJ->response['result'].= "</div>";
$appRJ->response['result'].= "</div>";
if($catCount>0){
    ...
    while ($selectCat_row=$DB->doFetchRow($selectCat_res)){
        ...
        $appRJ->response['result'].= "<div class='item-line-id'>";
        $appRJ->response['result'].= "<a href='/gallery/glManager/editCat/?cat_id=".$selectCat_row['glCat_id']."'>".$selectCat_row['glCat_id']."</a>";
        $appRJ->response['result'].= "</div>";
        $appRJ->response['result'].= "<div class='item-line-par_id'>";
