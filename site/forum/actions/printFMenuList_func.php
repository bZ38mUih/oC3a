<?php
function printFMenuList($artCatPar_id=null, $DB, $ref)
{
    $tmpRes['text']=null;
    $tmpRes['cntCat']=null;
    $tmpRes['cntItm']=null;
    ...
    if($catCount>0 or $artCount>0){
        ...
        if($artCount>0){
            while ($slArt_row=$DB->doFetchRow($slArt_res)){
                $tmpItm=null;
                $tmpItm.="<li class='itm-line'><a href='/".$ref."/".$slArt_row['sAlias']."'>".
                    "<div class='itm-line-img'>";
                if($slArt_row['sImg']){
                    $tmpItm.= "<img src='".F_SUBJ_IMG.$slArt_row['art_id']."/preview/".$slArt_row['sImg']."'>";
                }else{
        ...
        if($catCount>0){
            while ($selectCat_row=$DB->doFetchRow($selectCat_res)){
                ...
                $tmpCat.= "</div></div></a>";
                $tmpRes['text'].=$tmpCat;
                $responce=printFMenuList($selectCat_row['fm_id'], $DB, $ref);
    ...
    if($catCount>0 or $artCount>0){
        ...
        if($catCount>0){
            while ($selectCat_row=$DB->doFetchRow($selectCat_res)){
                ...
                $tmpCat.= "</div></div></a>";
                $tmpRes['text'].=$tmpCat;
                $responce=printFMenuList($selectCat_row['artCat_id'], $DB, $ref);
                $tmpRes['cntCat']=$catCount+$responce['cntCat'];
                $tmpRes['cntItm']+=$responce['cntItm'];
