...
$appRJ->response['result'].= "<meta name='description' content='Правка категории тем' http-equiv='Content-Type' charset='charset=utf-8'>";
...
if($Cat_rd->result['catImg']){
    $appRJ->response['result'].= "<img src='".FORUM_CATEG_IMG_PAPH.$_GET['cat_id']."/preview/".$Cat_rd->result['catImg']."'>";
...
$appRJ->response['result'].= "<input type='file' onchange='loadFiles(".$_GET['cat_id'].", ".'"'."cat_id".'"'.")' accept='image/jpeg,image/png,image/gif'>";
$appRJ->response['result'].= "</div>";
...
$appRJ->response['result'].= "<label for='subjCat_id'>cat_id:</label>";
$appRJ->response['result'].= "<input type='text' name='subjCat_id' value='".$Cat_rd->result['subjCat_id']."' disabled>";
$appRJ->response['result'].= "</div>";
$appRJ->response['result'].= "<div class='input-line'>";
...
/*select options-->*/
$categList_text="select subjCat_id, subjCat_parId, catName from subjectsMenu_dt WHERE subjCat_id<>".$Cat_rd->result['subjCat_id'].
    " ORDER BY catName ";
