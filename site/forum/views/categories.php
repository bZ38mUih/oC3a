<?php
$selectCat_query = "select * from subjectsMenu_dt";
...
$appRJ->response['result'].= "<a href='/forum/forumManager/newCat/'><img src='/source/img/create-icon.png'>Создать категорию</a>";
$appRJ->response['result'].= "</div>";
$appRJ->response['result'].= "</div>";
if($catCount>0){
    ...
    while ($selectCat_row=$DB->doFetchRow($selectCat_res)){
        ...
        $appRJ->response['result'].= "<div class='item-line-id'>";
        $appRJ->response['result'].= "<a href='editCat/?cat_id=".$selectCat_row['subjCat_id']."'>".$selectCat_row['subjCat_id']."</a>";
        $appRJ->response['result'].= "</div>";
        $appRJ->response['result'].= "<div class='item-line-par_id'>";
        if($selectCat_row['subjCat_parId']){
            $appRJ->response['result'].= "<a href='editCat/?cat_id=".$selectCat_row['subjCat_parId']."'>".$selectCat_row['subjCat_parId']."</a>";
        }else{
        ...
        if($selectCat_row['catImg']){
            $appRJ->response['result'].= "<img src='".FORUM_CATEG_IMG_PAPH.$selectCat_row['subjCat_id']."/preview/".$selectCat_row['catImg']."'>";
        }else{
